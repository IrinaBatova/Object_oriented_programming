...
class Product(BaseProduct, ReprMixin):
    ...
    def __init__(self, name, description, price, quantity, color) -> None:
        ...
        # Если пользователь создает товар (метод # __init__) с нулевым количеством, вызывать исключение ValueError,
        # которое будет выдавать сообщение: Товар с нулевым количеством не может быть добавлен. При этом прерывать
